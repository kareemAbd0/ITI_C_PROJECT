    ...
    while (1)
    {
        ...
        else if (screen_variable == admin_screen)
        {
            ...
            if (option == 1)
            {
                ...
                {
                    printf("Number of Book is Maximum\n");
                }
                getch();
                screen_variable = admin_screen;
            }
            else if (option == 2)
            {
                ...
                int book_flag = delete_book(book_data, book_count, book_data[book_option - 1].isbn);
                system("cls");
                if (book_flag == 1)
                {
                    printf("%s Book is deleted.\n",book_data[book_option - 1].title);
                }
                else if (book_flag == -1)
                {
                    ...
                    if (strcmp(borrowed_books_data[i].borrower_id, current_user_id) == 0)
                    {
                        for (int j = 0; j < book_count; j++)
                        {
                            if (strcmp(book_data[j].isbn, borrowed_books_data[i].isbn) == 0)
                            {
                                ...
                                title_ptrs[borrowed_book_user_count] = titles[borrowed_book_user_count];
